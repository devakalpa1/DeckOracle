...
import { useCreateDeckMutation } from '../store/services/api';
...
const CreateDeckModal = ({ isOpen, onClose, onSuccess }: CreateDeckModalProps) => {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [isPublic, setIsPublic] = useState(false);
  const [createDeck, { isLoading, error }] = useCreateDeckMutation();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-primaryDark">Create New Deck</h2>
          <button
            onClick={onClose}
